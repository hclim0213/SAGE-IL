...
class TransformerDecoderGenerator(AbstractGenerator):
    def __init__(
        self,
        n_token: int,
        n_embed: int,
    ...
    def forward(  # type: ignore
        ...
        output = self.decode(tgt=tgt, use_tgt_mask=use_tgt_mask)
        return output

    def decode(
        self, tgt: torch.Tensor, use_tgt_mask: bool
    ) -> torch.Tensor:
        if use_tgt_mask:
        ...
        dummy_memory = torch.zeros_like(tgt_embed)
        
        output = self.transformer_decoder(
            tgt_encoded, memory=dummy_memory, tgt_mask=self.tgt_mask
        )
