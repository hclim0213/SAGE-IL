...
RULES_FILENAME = "/workspace/_ext/data/assets/rd_filter/rules.json"
ALERT_FILENAME = "/workspace/_ext/data/assets/rd_filter/alert_collection.csv"
...
class RDFilter:
    ...
    def __call__(self, smiles: str):
        ...
        if not (
            self.rule_dict["HBD"][0] <= NumHDonors(mol) <= self.rule_dict["HBD"][1]
        ):
            return False

        if not (
            self.rule_dict["HBA"][0] <= NumHAcceptors(mol) <= self.rule_dict["HBA"][1]
        ):
            return False
